...
PACKAGE_BIN_DIR = os.path.abspath(os.path.join(CURRENT_DIR, "..", "package", "bin"))
AOB_PY3_DIR = os.path.abspath(
    os.path.join(CURRENT_DIR, "..", "bin", "ta_opencti_for_splunk_enterprise", "aob_py3")
)
if AOB_PY3_DIR not in sys.path:
    sys.path.insert(0, AOB_PY3_DIR)
if PACKAGE_BIN_DIR not in sys.path:
    sys.path.insert(0, PACKAGE_BIN_DIR)

try:
    from stix2.canonicalization.Canonicalize import canonicalize  # noqa: F401
except ImportError:
    stix2_module = types.ModuleType("stix2")
    canonicalization_module = types.ModuleType("stix2.canonicalization")
    canonicalize_module = types.ModuleType("stix2.canonicalization.Canonicalize")

    def canonicalize(data, utf8=False):
        _ = utf8
        return json.dumps(data, sort_keys=True, separators=(",", ":"))

    canonicalize_module.canonicalize = canonicalize
    sys.modules["stix2"] = stix2_module
    sys.modules["stix2.canonicalization"] = canonicalization_module
    sys.modules["stix2.canonicalization.Canonicalize"] = canonicalize_module
