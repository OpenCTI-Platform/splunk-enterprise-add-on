...
class AlertActionWorkeropencti_create_incident(ModularAlertBase):
    ...
    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkeropencti_create_incident, self).__init__(
            ta_name, alert_name
        )
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_opencti_create_incident_helper.process_event(
                self, *args, **kwargs
            )
...
if __name__ == "__main__":
    exitcode = AlertActionWorkeropencti_create_incident(
        "TA-opencti-for-splunk-enterprise", "opencti_create_incident"
    ).run(sys.argv)
    sys.exit(exitcode)
